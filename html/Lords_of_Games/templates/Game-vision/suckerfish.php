...
function mosGetLink( $mitem, $level, $class_sfx='' ) {
    ...
    if ($mitem->cnt > 0) {
		   if ($level == 0) {
                $txt = "<a class=\"main\" target=\"_window\"  href=\"$mitem->link\">$mitem->name</a>";
								$main = "main";
		   } else {
                $txt = "<a class=\"sub\" target=\"_window\"  href=\"$mitem->link\">$mitem->name</a>";
		   }
		} else {
    ...
    if ($mitem->cnt > 0) {
				if ($level == 0) {
                $txt = "<a href=\"#\" class=\"main\" onClick=\"javascript: window.open('$mitem->link', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=550');\" class=\"$menuclass\">$mitem->name</a>\n";
		   					$main = "main";
		} else {
                $txt = "<a href=\"#\" class=\"sub\" onClick=\"javascript: window.open('$mitem->link', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=550');\" class=\"$menuclass\">$mitem->name</a>\n";
		   }
		} else {
    ...
    if ($mitem->cnt > 0) {
		   if ($level == 0) {
                $txt = "<a class=\"main\">$mitem->name</a>";
								$main = "main";
		   } else {
                $txt = "<a class=\"sub\">$mitem->name</a>";
		   }
		} else {
		   	$txt = "<a>$mitem->name</a>\n";
		}
		...
		if (isset($mitem->cnt) && $mitem->cnt > 0) {
		    if ($level == 0) {
                $txt = "<a class=\"main\" href=\"$mitem->link\">$mitem->name</a>";
								$main = "main";
		   } else {
                $txt = "<a class=\"parent\" href=\"$mitem->link\"> $mitem->name</a>";
		   }
		} else {

if ($level == 0) {

		   $txt = "<a class=\"active\" href=\"$mitem->link\">$mitem->name</a>";

}

else if ($level > 0) {

		   $txt = "<a class=\"sub\" href=\"$mitem->link\">$mitem->name</a>";
    ...
    return "<span class=\"" . $main . "\">" . $txt . "</span>";
}
