...
class Gantry {
    ...
    function init() {
        ...
        if (array_key_exists('gantry_profile', $_GET)){
            $profile = JProfiler::getInstance('Gantry->init()');
            $profile->mark('Start');
        }
        ...
        $this->templateUrl = $this->baseUrl.'templates'."/".$this->_getCurrentTemplate();
        $this->baseUrl = JURI::base(true);
        $this->gantryUrl = $this->templateUrl."/lib/gantry";
    ...
    function getCurrentUrl($ignore=array()){
        return $this->document->base . $this->_rebuildQueryString($ignore);
    }
    ...
    function displayBodyTag() {
        ...
        require_once($this->templatePath.'/lib/gantry/html/layouts.php');
        if (file_exists($this->layoutPath)) require_once($this->layoutPath);
        ...
        $layoutMethod = 'docLayout_body';
		// Apply chrome and render module
		if (function_exists($layoutMethod)) {
			  return $layoutMethod(implode(" ", $body_classes), $this->_bodyId);
		}
		return "";
    }
	...
	function addScript($file = '') {
        ...
        foreach ($out_files as $script_path => $script_path) {
            $this->_scripts[$script_path] = $script_path;            
        }
	}
    ...
    function writeMenuItemParams($id, $data){
        ...
        if (file_exists($this->custom_menuitemparams_dir)){
            ...
            $menu_params_file = $this->_menuitem_params_path.DS.$id.'.menuparams.ini';
            if (is_array($data)){
     ...
     * @param  $name
     * @param  $preset
     * @return #M#CGantry._getPresetParams|array|?
     */
