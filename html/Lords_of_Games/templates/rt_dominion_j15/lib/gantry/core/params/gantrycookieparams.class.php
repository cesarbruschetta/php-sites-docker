...
class GantryCookieParams extends GantryParams {
    ...
    function store(){
        ...
        foreach($this->_setincookie as $cookie_var){
            if ($this->_working_params[$cookie_var]['value'] != $gantry->_working_params[$cookie_var]['sitebase']){
                setcookie($gantry->template_prefix.$gantry->_base_params_checksum."-".$cookie_var, $gantry->_working_params[$cookie_var]['value'], $cookie_time, '/', false);
            }
            else {
    ...
    function _flushOldCookies(){
        ...
        foreach($_COOKIE as $cookie_key=> $cookie_value){
            if (strpos($cookie_key, $gantry->template_prefix) === 0 && strpos($cookie_key, $gantry->template_prefix.$gantry->_base_params_checksum)===false) {
                setcookie($cookie_key, "", $cookie_time, '/', false);
            }
        }
    }
