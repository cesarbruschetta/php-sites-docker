...
class JElementPositions extends JElement {
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
        ...
        if (!defined('GANTRY_CSS')) {
			$document->addStyleSheet('../templates/'.$this->template.'/lib/gantry/admin/widgets/gantry.css');
			define('GANTRY_CSS', 1);
		}
		...
		if (!defined('POSITIONS')) {
			...
			if (!defined('GANTRY_SLIDER')) {
				$document->addScript('../templates/'.$this->template.'/lib/gantry/admin/widgets/slider/js/slider.js');
				define('GANTRY_SLIDER', 1);
			}
			$document->addScript('../templates/'.$this->template.'/lib/gantry/admin/widgets/slider/js/unserialize.js');
	...
	function tryCache($implode, $scheme, $words) {
		...
		$file = $gantry->gantryPath . "/admin/cache/$grid/" . $md5;
		if (!is_dir($gantry->gantryPath . "/admin/cache/$grid/")) mkdir($gantry->gantryPath . "/admin/cache/$grid/");
		...
		if (!file_exists($file)) {
			...
			$unserial = unserialize(file_get_contents($file));
	...
	function getCombinations() {
		...
		foreach($sets as $set) {
			...
			$md5 = md5($grid . $implode . $set);
			$file = $gantry->gantryPath . "/admin/cache/$grid/" . $md5;
			if (!file_exists($file)) file_put_contents($file, serialize($current));
		...
		window.addEvent('domready', function() {
			...
			window.slider$name2 = new RokSlider($slider, $knob, {
				...
				onDrag: function(now) {
					...
					var layout = this.RT.list[this.RT.current][step], output = '';
					...
					for (i = 0, len = this.RT.current; i < len; i++) {
						...
						if (this.RT.type == 'custom') {
							...
							this.RT.store[this.RT.current].push(this.RT.keys[this.RT.current][step][i]);
							
						} else {
							this.RT.settings[this.RT.current].push(layout.charAt(i));
						}
						if (this.RT.keys) {
							var keyIndex = this.RT.keys[this.RT.current][step][i];
	...
	function generalFunctions() {
		$js = "
			...
			var updateSlider = function(slider, range) {
				...
				if (!x.steps) {x.steps = 1; x.set(0);x.drag.detach();}
				else x.drag.attach();
			...
			var updateBlocks = function(slider, amount, step) {
				...
				blocks.each(function(block, i) {
					...
					var grid = slider.RT.list[amount][step].toString();
					blocks[i].className = '';
